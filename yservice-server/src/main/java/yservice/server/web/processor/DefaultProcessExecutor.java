...
import yservice.server.balancer.BalancerStrategy;
import yservice.server.balancer.RoundRobinBalancerStrategy;
...
public class DefaultProcessExecutor extends AbstractProcessExecutor {
	...
	/**
	 * Constructs a newly default processor executor
	 * defining {@code RoundRobinBalancerStrategy} as balancer strategy.
	 * 
	 * @see RoundRobinBalancerStrategy
	 */
	public DefaultProcessExecutor() {
		this(new RoundRobinBalancerStrategy());
	}
	
	/**
	 * Constructs a newly default processor executor
	 * defining a given balancer strategy.
	 * 
	 * @param balancerStrategy the balancer strategy
	 */
	public DefaultProcessExecutor(BalancerStrategy balancerStrategy) {
	...
	protected ResponseWrapper execute(RequestWrapper req, ProcessExecutorChain chain) throws IOException {
		...
		WebTarget target = client.target(serviceRegistry.getDomain()).path(req.getUri());
